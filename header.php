...
<?php if(!has_post_thumbnail( $post->ID )) { $default_image = of_get_option('facebook_image'); 
echo '<meta name="thumbnail" content="' . $default_image . '" />
  <meta name="twitter:image:src" content="' . $default_image . '">
  <meta property="og:image" content="' . $default_image . '">'; } else { $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' ); echo '<meta name="thumbnail" content="' . esc_attr( $thumbnail_src[0] ) . '" />
  <meta name="twitter:image:src" content="' . esc_attr( $thumbnail_src[0] ) . '">
  <meta property="og:image" content="' . esc_attr( $thumbnail_src[0] ) . '">'; } echo "\n"; ?><?php } ?>
...
<?php global $sub_slides; 
	
$license 	= get_option( 'edd_form_license_key' );
$status 	= get_option( 'edd_form_license_status' );
if ($status !== false && $status == 'valid' ) { 
	

	if ($sub_slides['slide_first'] == '1') { if (function_exists('ab_sub_form'))  ab_sub_form();  if (function_exists('ab_sub_slides'))  ab_sub_slides(); } else  { if (function_exists('ab_sub_slides'))  ab_sub_slides();  if (function_exists('ab_sub_form'))  ab_sub_form();  } } ?>
